      ...
      <v-navigation-drawer v-model="drawer" :rail="rail" permanent @click="rail = false" color="indigo darken-1">
        <v-list-item>
          <template v-if="!isAuthenticated">
            <v-btn prepend-icon="mdi-login" class="bg-red-darken-1 ma-1" width="7rem" v-bind:to="{ name: 'login' }">
              LogIn
            </v-btn>
            <v-btn prepend-icon="mdi-login" class="bg-red-darken-1 ma-1" width="7rem" v-bind:to="{ name: 'signup' }">
              SignUp
            </v-btn>
          </template>
          <template v-else>
            <v-list-item-title class="text-h6 white--text">{{ user.username }}</v-list-item-title>
          </template>
          <template v-slot:append>
